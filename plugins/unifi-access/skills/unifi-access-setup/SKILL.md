...
description: Configure the UniFi Access MCP server for Claude Code, Codex, Copilot CLI, or OpenClaw — set controller host, credentials, API key, and permissions
...
```bash
bash <path-to-plugin>/scripts/check-prereqs.sh --target <claude|codex|copilot|openclaw> "unifi-access"
```

On Windows PowerShell:

```powershell
powershell -ExecutionPolicy Bypass -File <path-to-plugin>/scripts/check-prereqs.ps1 -Target <claude|codex|copilot|openclaw> -PluginName "unifi-access"
```
...
- Copilot CLI target: replaces the `unifi-access` MCP server via `copilot mcp add --env ... -- uvx ...`
...
"Copilot CLI MCP server `unifi-access` configured in `~/.copilot/mcp-config.json`. Restart Copilot CLI, or run `/mcp` to reload, so the updated MCP server is loaded."
