...
- **A plugin-scoped MCP server shadows same-named user and workspace entries**, and cannot be overridden by `copilot mcp add`. Copilot resolves plugin MCP servers from `plugins/<name>/.mcp.json` when that file exists, and **falls back to the `mcpServers` block in `.claude-plugin/plugin.json` when it does not** (verified on Copilot CLI 1.0.75). That fallback registers Claude's `${VAR:-default}` env values unexpanded and the server dies at startup. Each plugin therefore ships an explicit empty `plugins/<name>/.mcp.json` (`{"mcpServers": {}}`) to block it, and the Codex server definition lives at `plugins/<name>/.codex-plugin/mcp.json`. `scripts/smoke-plugin-setup.sh` asserts both — the file exists, and neither it nor `.github/mcp.json` declares any server (run by `make smoke-plugins`, part of `make check` and the `Test: Plugin setup scripts` workflow).
