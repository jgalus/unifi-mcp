...
Register the marketplace, then install `unifi-network`:
...
After installing, ask Copilot CLI to use the UniFi Network setup skill. The setup flow registers the MCP server with `copilot mcp add` at user scope (`~/.copilot/mcp-config.json`) and prompts you to restart Copilot CLI (or run `/mcp`).
